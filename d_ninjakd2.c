...
//			UINT32 off2s = sx | (address/0x40) <<6;
			UINT32 off2s = sx | (address & 0xffc0);
...
//		Next = (UINT8 *)0x2F5000;
		addr_cache		= (UINT32 *)Next; Next += 0x1000 * (sizeof(UINT32));
//		addr_cache		= (UINT32 *)0x2F6000;
	}
	else
	{
...
INT32 RobokidInit()
{
	...
	memset(&addr_cache[0], 0, 0x1000*sizeof(UINT32));
	memset((void *)0x2F6000, 0, 0x2000*sizeof(UINT32));
//	bg_cache[0] =&addr_cache[0];
//	bg_cache[1] =(UINT32 *)0x2F6000;
//	bg_cache[2] =(UINT32 *)0x2FA000;	
...
UINT32 cacheTile(UINT8 *bgram,UINT32 *nt, UINT32 *bg1, UINT8 *src,UINT8 *dst)
{
	...
	UINT32 *new_code=(UINT32 *)&bg1[code];
	
	if(*new_code==0)
	{
		*new_code=(*nt<<2);
		UINT8 *addr = (UINT8*)src+(code*128);
		memcpyl((UINT8 *)dst+*new_code*32,addr,128);
		if((*nt)++>0xfff)
			*nt=0;
	}
	return *new_code;
...
//	UINT32 wide = (width) ? 128 : 32;
	...
	UINT16 code;
	register UINT16 *map;
	UINT16 *ml=(UINT16 *)map_lut;
	UINT16 *ml2=(UINT16 *)&map_lut[1024];
...
//-----------------------------------------------------------------------------------------------------------------
// nbg1
		code = cacheTile(bgram,nt,addr_cache,(UINT8*)LOWADDR,(UINT8 *)SS_CACHE+0x08000);
...
// nbg3
		...
		code = cacheTile(bgram,nt,(UINT32 *)0x2FA000,(UINT8*)DrvGfxROM4Data1,(UINT8 *)SS_CACHE+0x48000);
...
// nbg0	
		...
		code = cacheTile(bgram,nt,(UINT32 *)0x2F6000,(UINT8*)0x00270000,(UINT8 *)SS_CACHE+0x28000);
...
inline void make_lut()
{
	...
	for(UINT16 i=0;i<1024;i++)
	{
		...
		UINT16 ofst = (sx & 0x0f) + (sy * 16) + ((sx & 0x70) * 0x20);
		map_lut[i]= ofst<<1;
