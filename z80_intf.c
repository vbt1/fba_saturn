...
//	DebugCPU_ZetInitted = 1;
	...
	nOpenedCPU = -1;
FNT_Print256_2bppSel((volatile Uint8 *)0x25e20000,(Uint8 *)"malloc zet               ",16,150);

	ZetCPUContext[nCPU] = (struct ZetExt*)BurnMalloc(sizeof(ZetExt));
	memset (ZetCPUContext[nCPU], 0, sizeof(ZetExt));

	if (nCPU == 0) { // not safe!
//FNT_Print256_2bppSel((volatile Uint8 *)0x25e20000,(Uint8 *)"nCPU == 0               ",16,150);

		Z80Init();
	}
//FNT_Print256_2bppSel((volatile Uint8 *)0x25e20000,(Uint8 *)"nCPU == 0 done               ",16,150);
