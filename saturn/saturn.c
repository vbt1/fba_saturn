...
#define CSH_4WAY			((Uint16)0x0000)	/*	4?????i????Ë?????	*/
#define CSH_2WAY			((Uint16)0x0008)	/*	2?????i????Ë?????	*/
...
{
	typedef Uint32 Line[0x10/sizeof(Uint32)];	/* ???C???? 0x10 ?o?C?g?P?? */
	Line *ptr, *end;
	Uint32 zero = 0;

	ptr = (void*)(((Uint32)adrs & 0x1fffffff) | 0x40000000);	/* ?L???b?V???p?[?W?®? */
	end = (void*)((Uint32)ptr + P_size - 0x10);	/* ?I???|?C???^?i-0x10 ??|?X?g?C???N???????g?®?j */
	ptr = (void*)((Uint32)ptr & -sizeof(Line));	/* ???C???A???C?????g????? */
	do {
...
//-------------------------------------------------------------------------------------------------------------------------------------
static unsigned short get_hz(void)
{
	if((TVSTAT & 1) == 0)
		return 60;
	else
