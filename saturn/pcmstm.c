...
void	operate_adx_stream(void)
{
	...
	} else if(adx_stream.active && adx_stream.playing)
	{
		...
		if(m68k_com->adx_buffer_pass[0])
		{
			...
			memcpyl(adx_stream.back_buffer[0], adx_stream.front_buffer[0], (1024 * 9));
		...
		if(m68k_com->adx_buffer_pass[1])
		{
			...
			//This is 9KB of data (half of 18KB buffer).
			memcpyl(adx_stream.back_buffer[1], adx_stream.front_buffer[1], (1024 * 9));
...
//	cd_init();
	...
	*/
		...
		{
//FNT_Print256_2bppSel((volatile Uint8 *)SS_FONT,(Uint8 *)"buf.setup_requested      --",160/2, 110);				
				//game_code();
			/////////////
					//jo_printf(16, 2, "--SETM--");
...
//			m68k_com->pcmCtrl[numberPCMs].bytes_per_blank = 256;
m68k_com = (sysComPara *)((SNDPRG + DRV_SYS_END) | 0x20000000);		
			...
			//////////////
				//slSynch();
//				wait_vblank();
//FNT_Print256_2bppSel((volatile Uint8 *)SS_FONT,(Uint8 *)"buf.setup_requested end     --",160/2, 110);	
		} else if(stm.stopping)
		{
//FNT_Print256_2bppSel((volatile Uint8 *)SS_FONT,(Uint8 *)"stm.stopping                --",160/2, 120);				
				//game_code();
			buf.operating = false;
			...
			if(stm.restarting) 
			{
//FNT_Print256_2bppSel((volatile Uint8 *)SS_FONT,(Uint8 *)"stm.restarting                --",160/2, 120);				
				buf.setup_requested = true;
				stm.restarting = false;
				stm.playing = false;
				pcm_cease(stm.pcm_num);
			} else {
//FNT_Print256_2bppSel((volatile Uint8 *)SS_FONT,(Uint8 *)"stm.playing = false                --",160/2, 120);				
				stm.playing = false;
				pcm_cease(stm.pcm_num);
			}
...
//				wait_vblank();				
		...
		//Broadly speaking, this branch is generic file system or ADX streaming in the PCM stream's free time.
			...
			//This branch is for no file system activity. Nothing is being presently accessed or requested to be set up.
				game_code();
//FNT_Print256_2bppSel((volatile Uint8 *)SS_FONT,(Uint8 *)"--PLAY--",160/2, 20);				
					//jo_printf(16, 2, "--PLAY--");
...
//				wait_vblank();
			...
			file.destination + (file.sectors_read_so_far * file_transfer_size), file_transfer_size);
				do{
					game_code();
//FNT_Print256_2bppSel((volatile Uint8 *)SS_FONT,(Uint8 *)"--FILE--",160/2, 20);					
						//jo_printf(16, 2, "--FILE--");
...
//char toto[50];
//m68k_com = (sysComPara *)((SNDPRG + DRV_SYS_END) | 0x20000000);
//FNT_Print256_2bppSel((volatile Uint8 *)SS_FONT,(Uint8 *)"--MUSI--",160/2, 20);	
//sprintf(toto,"loop %d total %d %08x %d %d nb%d      ",	stm.times_to_loop,stm.total_blanks,m68k_com,m68k_com->pcmCtrl[0].bytes_per_blank,m68k_com->pcmCtrl[1].bytes_per_blank,stm.pcm_num);
//FNT_Print256_2bppSel((volatile Uint8 *)SS_FONT,(Uint8 *)toto,10, 30);				
//			stm.total_blanks = buf.total_bytes / (int)m68k_com->pcmCtrl[stm.pcm_num].bytes_per_blank;	
//			m68k_com->pcmCtrl[stm.pcm_num].bytes_per_blank = 256;
			
//sprintf(toto,"btot %d byt %d         ",	buf.total_bytes,(int)m68k_com->pcmCtrl[stm.pcm_num].bytes_per_blank);
//FNT_Print256_2bppSel((volatile Uint8 *)SS_FONT,(Uint8 *)toto,10, 40);	
		
					//jo_printf(16, 2, "--MUSI--");
					// jo_printf(2, 5, "steps(%i)", buf.steps_of_new_data_in_buffer);
//sprintf(toto,"steps(%i)", buf.steps_of_new_data_in_buffer);					
//FNT_Print256_2bppSel((volatile Uint8 *)SS_FONT,(Uint8 *)toto,20, 50);						
					// jo_printf(2, 6, "bytes(%i)", bytes_read_now);
//sprintf(toto,"act(%i)", buf.active_buf_segment);					
//FNT_Print256_2bpp((volatile Uint8 *)SS_FONT,(Uint8 *)toto,160/2, 50);					
					// jo_printf(16, 5, "act(%i)", buf.active_buf_segment);
//sprintf(toto,"bufrq(%i)", buf.needs_buffer_filled);					
//FNT_Print256_2bpp((volatile Uint8 *)SS_FONT,(Uint8 *)toto,160/2, 60);						
					// jo_printf(16, 6, "bufrq(%i)", buf.needs_buffer_filled);
//sprintf(toto,"buf0f(%i)", buf.segment_full[0]);					
//FNT_Print256_2bpp((volatile Uint8 *)SS_FONT,(Uint8 *)toto,20, 80);					
					// jo_printf(2, 8, "buf0f(%i)", buf.segment_full[0]);
//sprintf(toto,"buf1f(%i)", buf.segment_full[1]);					
//FNT_Print256_2bpp((volatile Uint8 *)SS_FONT,(Uint8 *)toto,20, 90);					
					// jo_printf(2, 9, "buf1f(%i)", buf.segment_full[1]); 
//sprintf(toto,"buf2f(%i)", buf.segment_full[2]);					
//FNT_Print256_2bpp((volatile Uint8 *)SS_FONT,(Uint8 *)toto,20, 100);					
					// jo_printf(2, 10, "buf2f(%i)", buf.segment_full[2]);
//sprintf(toto,"stat(%i)", gfs_svr_status);					
//FNT_Print256_2bpp((volatile Uint8 *)SS_FONT,(Uint8 *)toto,10, 190);						
					// jo_printf(0, 19, "stat(%i)", gfs_svr_status);
//sprintf(toto,"steps(%i)", buf.vblank_counter);					
//FNT_Print256_2bpp((volatile Uint8 *)SS_FONT,(Uint8 *)toto,20, 50);						
					// jo_printf(2, 5, "steps(%i)", buf.vblank_counter);
					// jo_printf(2, 6, "bytes(%i)", buf.active_buf_segment);
					// jo_printf(2, 8, "buf0f(%i)", buf.segment_full[0]); 
					// jo_printf(2, 9, "buf1f(%i)", buf.segment_full[1]); 
...
//	FNT_Print256_2bppSel((volatile Uint8 *)SS_FONT,(Uint8 *)"-- while (1) ended                --",160/2, 80);	
}
