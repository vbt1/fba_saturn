...
/*		
		int s = 0x0000 >> CZ80_FETCH_SFT;
		int e = (0xffff + CZ80_FETCH_BANK - 1) >> CZ80_FETCH_SFT;	
		
		for (int j = s; j < e; j++) {
			CZetCPUContext[i].Read[j] = NULL;
			CZetCPUContext[i].Write[j] = NULL;			
			CZetCPUContext[i].Fetch[j] = NULL;
			CZetCPUContext[i].FetchData[j] = NULL;
		}	*/		
	}	
